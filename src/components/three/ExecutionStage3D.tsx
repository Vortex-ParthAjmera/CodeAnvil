...
export function ExecutionStage3D({ step, steps }: { step: TraceStep; steps?: TraceStep[] }) {
  ...
  if (dispatch.kind === "array" && isMajorityVoteTraceStep(step)) {
    return <MajorityVoteStage3D step={step} steps={steps} />;
  }
  ...
  if (dispatch.kind === "array" && isThreeSumTraceStep(step)) {
    return <ThreeSumStage3D step={step} steps={steps} />;
  }
  ...
  if (dispatch.kind === "array" && isMinArrayTraceStep(step)) {
    return <MinArrayStage3D step={step} steps={steps} />;
  }

  if (dispatch.kind === "array" && isReverseArrayTraceStep(step)) {
    return <ReverseArrayStage3D step={step} steps={steps} />;
  }

  if (dispatch.kind === "array" && isKadaneTraceStep(step)) {
    return <KadaneStage3D step={step} steps={steps} />;
  }
