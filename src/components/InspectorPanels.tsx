...
function VarRow({ name, value, changed }: { name: string; value: unknown; changed: boolean }) {
  return (
    <div
      key={`${name}-${changed}`}
      className={cn(
...
function MemoryArray({ item }: { item: MemoryItem }) {
  return (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-1">
              ...
              >
                ...
                <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
                  {Object.entries(frame.locals).map(([name, value]) => (
                    <span key={name} className="font-mono text-[11px] text-ink-300">
        ...
        {tab === "memory" && (
          <div>
            ...
            {step.memory?.map((item) => <MemoryArray key={item.id} item={item} />)}
          </div>
        )}
